    ...
    <section id="casos" className="py-20 px-4 sm:px-6 lg:px-8 bg-card/40 border-t border-border/30">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-10">
          {t.cases.items.map((item, i) => {
            ...
            return (
              <article
                key={item.title}
                className="flex flex-col gap-4 p-6 bg-background border border-border/40 rounded-xl"
              >
                ...
                <div className="mt-auto pt-4 border-t border-border/40">
                  ...
                  <p className="text-sm font-bold text-primary leading-snug">{item.roi}</p>
                </div>
              </article>
            )
          })}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 px-5 py-2.5 border border-border hover:border-primary text-foreground hover:text-primary rounded-lg font-bold text-sm transition-colors"
          >
            {t.cases.cta}
